...
function findWorker($areaName) {
	global $workers, $areas, $nearby;
	$areasByWorkerIndex = array_flip(array_column($workers, 'area_name'));
    if (isset($areasByWorkerIndex[$areaName])) {
    	$workerIndex = $areasByWorkerIndex[$areaName];
    	return $workers[$workerIndex]['login'];
    } else {
    	$areaIndex = array_search($areaName, $areas);
        $nerbyIndexes = $nearby[$areaIndex];
        $nearestWokers = [];
        foreach ($nerbyIndexes as $index) {
        	$nerbyName = $areas[$index];
        	if (isset($areasByWorkerIndex[$nerbyName])) {
            	$workerIndex = $areasByWorkerIndex[$nerbyName];
    			$nearestWokers[] = $workers[$workerIndex]['login'];
            }
        }
        return count($nearestWokers) ? $nearestWokers : null;
    }
}
